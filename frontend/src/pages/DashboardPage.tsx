...
const DashboardPage: React.FC = () => {
	...
	const selectedPlayerAverageRating = useMemo(() => {
		...
		const total = playerPerformances.reduce(
			(sum, perf) => sum + perf.rating,
			0,
		);
	...
	const loadSelectedPlayer = useCallback(async (playerId: number) => {
		...
		try {
			const performancesData =
				await performanceService.getPerformancesByPlayer(playerId);
			...
			>
				<div>
					...
					<div style={{ color: "#666", marginTop: "6px" }}>
						Players, matches, and performance tracking
					</div>
				</div>
			</div>
			...
			>
				...
				>
					...
					<PerformanceForm
						matches={matches}
						players={players}
						onPerformanceCreated={onPerformanceCreated}
					/>
				...
				>
					...
					>
						...
						<div style={{ color: "#666", marginTop: "4px" }}>
							Average rating:{" "}
							{selectedPlayerAverageRating == null
								? "-"
								: `${selectedPlayerAverageRating}/10`}
						</div>
					</div>
